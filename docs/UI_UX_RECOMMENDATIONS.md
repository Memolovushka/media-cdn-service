...
2. **Два режима просмотра: список и медиа-сетка**
   Сейчас файловый менеджер ближе к таблице. Для изображений и видео нужен grid-view с крупными превью, размером, CDN-статусом и быстрым выбором. Список оставить для документов, больших наборов файлов и точной работы. **Done:** file manager имеет переключатель List/Grid, image thumbnails, file type markers, CDN state и selection support в обоих режимах.

3. **Preview-first правую панель**
   Сделать правую панель главным местом работы с выбранным ассетом: крупное превью сверху, ниже имя, размер, MIME, CDN URL, действия. Для изображений, видео, аудио и PDF панель должна показывать реальный контент, а не только метаданные. **Done:** asset details panel начинается с preview surface и затем показывает inline rename, MIME/size, download, CDN lifecycle, Public URL и embed snippets.

4. **Breadcrumb как интерактивная навигационная строка**
   Путь папки должен ощущаться как адресная строка: каждый сегмент кликабельный, рядом компактная кнопка копирования пути, поддержка перехода клавиатурой. Это усиливает ощущение OS-like file browser. **Done:** file area header теперь показывает `Main / folder / child` как кликабельную address bar с client-side переходами по сегментам и compact copy-path кнопкой.
...
8. **Undo toast для destructive actions**
   После удаления файла/папки показывать toast с `Undo` на 5-10 секунд. Это снижает страх перед быстрыми действиями и делает интерфейс более зрелым. Backend уже использует soft-delete, поэтому UX хорошо ложится на модель. **Done for files:** file delete показывает toast с Undo на soft-delete restore endpoint; recursive folder undo оставлен вне этой правки, чтобы не восстанавливать лишние nested assets небезопасно.

9. **Инлайн-переименование без отдельной кнопки Save**
   Переименование можно сделать как в файловых менеджерах: `F2` или клик по имени, Enter сохраняет, Escape отменяет, blur сохраняет. Кнопку Save в правой панели лучше заменить на нативный inline editing pattern. **Done:** right panel filename field теперь работает без отдельной Save-кнопки: `F2` фокусирует имя, Enter/blur сохраняют, Escape отменяет локальное изменение.
